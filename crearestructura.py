...
def crear_estructura_proyecto():
    ...
    for origen, destino in archivos_a_mover.items():
        if os.path.exists(origen):
            ...
            try:
                ...
                respuesta = input(f"¿Deseas eliminar el archivo original {origen}? (s/n): ")
                if respuesta.lower() == 's':
                    os.remove(origen)
    ...
    if not os.path.exists('requirements.txt'):
        with open('requirements.txt', 'w') as f:
            f.write("""# Dependencias del proyecto Ñemongeta
opencv-python>=4.5.0
mediapipe>=0.8.9
numpy>=1.20.0
tensorflow>=2.6.0
...
setup(
    name="nembogueta",
    version="0.1.0",
    description="Módulo Python para reconocimiento de Lenguaje de Señas Paraguayo (LSPy)",
    author="Tu Nombre",
...
if __name__ == "__main__":
    ...
    if not os.path.exists('README.md'):
        print("⚠️ ADVERTENCIA: No se encontró README.md en el directorio actual.")
        respuesta = input("¿Estás seguro de que estás en el directorio raíz del proyecto? (s/n): ")
        if respuesta.lower() != 's':
            print("Operación cancelada.")
            sys.exit(1)

    # Confirmar antes de proceder
    print("Este script creará la estructura de directorios recomendada y moverá los archivos existentes.")
