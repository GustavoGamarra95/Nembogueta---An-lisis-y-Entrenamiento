"""
Script de reconocimiento en tiempo real para ALFABETO (Lenguaje de Señas).
Captura video desde webcam y reconoce letras en vivo.

Uso:
    python scripts/realtime_alphabet.py --model /models/alphabet/run_xxx/final_model.h5 \
...
class RealtimeAlphabet:
    """Sistema de reconocimiento en tiempo real para alfabeto."""
    ...
    def __init__(
        ...
        """
        Inicializa el sistema de reconocimiento en tiempo real de alfabeto.
        ...
        """
        # Cargar modelo
        logger.info(f"Cargando modelo de alfabeto desde {model_path}...")
        ...
        logger.info(f"Letras: {self.num_classes} ({', '.join(sorted(self.label_names))})")
        ...
        # Historial de letras reconocidas (para formar palabras)
        self.word_buffer = []
    ...
    def extract_landmarks_from_frame(self, frame: np.ndarray) -> Tuple[List[float], bool]:
        ...
        # Extraer landmarks de la PRIMERA mano solamente (63 features)
        frame_landmarks = []
        ...
        # Asegurar exactamente 63 features (1 mano)
    ...
    def predict(self, top_k: int = 3) -> List[Tuple[str, float]]:
        """
        Realiza predicción de letra en la secuencia actual.

        Args:
            top_k: Número de predicciones top

        Returns:
            Lista de (letra, probabilidad)
        """
    ...
    def draw_ui(self, frame: np.ndarray, predictions: List[Tuple[str, float]]) -> np.ndarray:
        ...
        # Panel de información superior (fondo semi-transparente)
        overlay = frame.copy()
        cv2.rectangle(overlay, (10, 10), (w - 10, 240), (0, 0, 0), -1)
        frame = cv2.addWeighted(overlay, 0.6, frame, 0.4, 0)

        # Estado de grabación
        status_text = "GRABANDO LETRA" if self.recording else "ESPERANDO"
        ...
        # Predicciones de letras
        if predictions:
            y_offset = 130
            cv2.putText(frame, "PREDICCIONES:", (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
            y_offset += 35

            for i, (letter, prob) in enumerate(predictions[:3]):
                # Letra grande para la mejor predicción
                if i == 0:
                    text = f"'{letter}' ({prob*100:.1f}%)"
                    color = (0, 255, 0)
                    font_size = 1.2
                    thickness = 3
                else:
                    text = f"{i+1}. '{letter}': {prob*100:.1f}%"
                    color = (255, 255, 255)
                    font_size = 0.6
                    thickness = 2

                cv2.putText(frame, text, (30, y_offset), cv2.FONT_HERSHEY_SIMPLEX, font_size, color, thickness)
                y_offset += 35 if i == 0 else 25

        # Palabra formada (panel derecho)
        if self.word_buffer:
            word_text = ''.join(self.word_buffer)
            overlay2 = frame.copy()
            cv2.rectangle(overlay2, (w - 310, 10), (w - 10, 100), (50, 50, 50), -1)
            frame = cv2.addWeighted(overlay2, 0.7, frame, 0.3, 0)

            cv2.putText(frame, "PALABRA:", (w - 300, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
            cv2.putText(frame, word_text, (w - 300, 75), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    ...
    def run(self):
        ...
        logger.info("Iniciando reconocimiento de alfabeto en tiempo real...")
        logger.info("Presiona ESPACIO para iniciar/detener grabación de letra")
        logger.info("Presiona 'c' para limpiar la palabra formada")
        logger.info("Presiona BACKSPACE para borrar la última letra")
        ...
        try:
            while True:
                ...
                # Mostrar frame
                cv2.imshow('Alfabeto - Reconocimiento en Tiempo Real', frame)
                ...
                if key == ord('q'):
                    ...
                    if self.recording:
                        logger.info("Iniciando grabación de letra...")
                        ...
                        if len(self.sequence_buffer) >= 10:
                            ...
                            if predictions:
                                best_letter = predictions[0][0]
                                best_prob = predictions[0][1]
                                logger.info(f"Letra reconocida: '{best_letter}' ({best_prob*100:.1f}%)")

                                # Añadir a la palabra si confianza > 50%
                                if best_prob > 0.5:
                                    self.word_buffer.append(best_letter)
                                    logger.info(f"Palabra actual: {''.join(self.word_buffer)}")

                                # Mostrar todas las predicciones
                                logger.info("Otras posibilidades:")
                                for i, (letter, prob) in enumerate(predictions[1:], 2):
                                    logger.info(f"  {i}. '{letter}': {prob*100:.1f}%")
                    ...
                    if self.word_buffer:
                        ...
                        logger.info(f"Letra '{removed}' eliminada")
                        logger.info(f"Palabra actual: {''.join(self.word_buffer) if self.word_buffer else '(vacía)'}")
    ...
    def cleanup(self):
        ...
        if self.word_buffer:
            logger.info(f"Palabra final formada: {''.join(self.word_buffer)}")
        self.cap.release()
...
def main():
    parser = argparse.ArgumentParser(
        description='Reconocimiento en tiempo real de ALFABETO'
    )
    ...
    realtime_system = RealtimeAlphabet(
