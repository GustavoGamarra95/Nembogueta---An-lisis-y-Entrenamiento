"""
Script de inferencia para ALFABETO (Lenguaje de Señas).
Carga un modelo entrenado de letras y realiza predicciones en videos individuales.

Uso:
    # Video individual
    python scripts/inference_alphabet.py --model /models/alphabet/run_xxx/final_model.h5 \
...
class AlphabetInference:
    """Clase para realizar inferencia con modelos de alfabeto."""

    def __init__(self, model_path: str, model_info_path: str):
        """
        Inicializa el sistema de inferencia de alfabeto.
    ...
    def predict(self, video_path: str, top_k: int = 5) -> List[Tuple[str, float]]:
        """
        Realiza predicción de letra en un video.
        ...
        Returns:
            Lista de tuplas (letra, probabilidad) ordenadas por confianza
        """
...
def main():
    parser = argparse.ArgumentParser(
        description='Inferencia con modelo de ALFABETO'
    )
    ...
    inference = AlphabetInference(args.model, args.model_info)
    ...
    if args.video:
        ...
        for i, (letter, prob) in enumerate(predictions, 1):
            logger.info(f"{i}. Letra '{letter}': {prob*100:.2f}%")
    ...
    if args.output:
        ...
        output_data = {
            video: [(letter, prob) for letter, prob in preds]
